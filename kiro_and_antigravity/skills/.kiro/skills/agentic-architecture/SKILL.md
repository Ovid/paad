...
Write verified findings to `.reviews/architecture/<YYYY-MM-DD>-<repo-slug>-architecture-report.md`.
...
- **Writable check:** verify `.reviews/architecture/` is writable before producing the report. If the directory exists but is not writable, abort Phase 4 with a clear message naming the directory and exit code; do **not** proceed to assemble the report content only to fail at write time.
