...
Loaded when `.reviews/test-roadmap/test-roadmap.md` already exists — the path every run after
...
6. **Verify the phase touched no production code, then latch.** Before writing
   ...
   code and *writes* under `.reviews/test-roadmap/` — no existing production code
...
`.reviews/test-roadmap/`. It may never modify, delete, move, or add production code.
...
- **Separate-file ecosystems** (Python, JS, Go's `_test.go`, Ruby, PHP, Perl):
  every changed path must be a test file or under `.reviews/test-roadmap/`.
...
- The obstacle is also recorded as `scaffold`-class debt in the ledger /
  `.reviews/test-roadmap/test-suite-analysis.md`, naming the refactor (usually a dependency-
...
it clears the inclusion gate, record it in `.reviews/test-roadmap/test-roadmap-findings.md`.
...
Create `.reviews/test-roadmap/test-roadmap-findings.md` if it does not yet exist (build mode
...
writing these; they're in `.reviews/test-roadmap/test-roadmap-findings.md`, each with the test
