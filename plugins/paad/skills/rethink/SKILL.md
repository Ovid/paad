...
**On invocation:** announce "Running paad:rethink v1.30.1" before anything else.
...
- **No options are in play.** If nobody has proposed anything, there is no reasoning to check. Use `/paad:pushback` on the spec instead.
...
- `/paad:rethink` — re-examine the most recent set of options in the conversation
- `/paad:rethink the caching approach` — name which decision, when several are live
...
Dispatch **one** agent using the Agent tool with `subagent_type: paad:paad-analyst` to check the premises.
