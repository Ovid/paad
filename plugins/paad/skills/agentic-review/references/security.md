...
> **Read this file before producing findings.** You are the Security specialist dispatched by `/paad:agentic-review` Phase 2. Your standing instructions in the parent `SKILL.md` cover the inputs you receive and the basic finding-report format. This file covers the Security lens specifically.
...
If the diff touches no trust boundary (pure UI, styling, internal refactor with no new I/O, test-only changes), output "Security: no security-relevant changes in this diff" and stop. Do not invent risks.
