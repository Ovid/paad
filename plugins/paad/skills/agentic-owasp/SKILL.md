...
5. **No exploitation, and no live systems.** This skill reviews source. It does
   not start the application, connect to a database, send a request to any
   host, or write a proof-of-concept that executes. If the user asks for a
   live test, tell them this skill does not do that and that testing a running
   system needs an authorization scope they own.
